...
export default function ReportsDashboard() {
  ...
  const [archivedReports, _setArchivedReports] = useState<Report[]>([]);
  ...
  const _fetchArchivedReports = async () => {};
          ...
          <div className="flex justify-between items-center px-8 pb-4">
            <div className="flex items-center gap-3">
              ...
              <div className="relative w-72">
                ...
                <input type="text" placeholder="Search reports..." className="w-full rounded-md border border-border pl-9 pr-3 py-2 bg-bg-light shadow-sm text-sm focus:border-primary focus:ring-2 focus:ring-primary/50 outline-none" />
              </div>
                  ...
                  return (
                    <tr key={report.id} className="border-b border-border">
                      ...
                      <td className="p-3 py-4 text-right pr-6">
                        {showingArchived ? (
                          <button className="text-xs text-info/90 hover:text-info font-medium" onClick={() => openModal('unarchive')}>Unarchive</button>
                        ) : (
                          <button
                            className={`text-xs font-medium ${report.canArchive ? 'text-error/90 hover:text-error' : 'text-text-muted/70 cursor-not-allowed'}`}
                            onClick={report.canArchive ? () => openModal('archive') : undefined}
                            disabled={!report.canArchive}
                          >
                            Archive
                          </button>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-md bg-bg-light rounded-lg shadow-lg p-6">
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button className="bg-error text-white rounded-md text-sm font-semibold px-4 py-2 hover:bg-red-700" onClick={() => closeModal('archive')}>Archive</button>
            </div>
          </div>
        </div>
      )}
