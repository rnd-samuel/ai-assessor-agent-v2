...
-- 1. Alter the project_prompts table
ALTER TABLE project_prompts 
DROP COLUMN IF EXISTS analysis_prompt,
ADD COLUMN IF NOT EXISTS kb_fulfillment_prompt TEXT,
ADD COLUMN IF NOT EXISTS competency_level_prompt TEXT,
ADD COLUMN IF NOT EXISTS development_prompt TEXT;

-- 2. Update the Default Prompts in system_settings
-- We use the prompts you provided in the prompt description.
INSERT INTO system_settings (key, value, updated_at)
VALUES (
  'default_prompts',
  '{
    "persona": "You are an expert Industrial & Organizational Psychologist and Lead Assessor with 20 years of experience in competency-based assessment centers. Your role is to evaluate a candidate (''asesi'') objectively, strictly following the evidence provided.\n\n**Core Guidelines:**\n1. **Objectivity:** Base all judgments solely on the provided evidence. Do not hallucinate or assume traits not demonstrated.\n2. **Language:** Output must be in simple, professional, semi-formal Bahasa Indonesia. Avoid academic jargon. Use ''Asesi'' instead of ''Kandidat'' or ''Peserta''.\n3. **Tone:** Constructive, psychological, and evidence-based.",
...
-- Add a column to track which phase is currently active (1, 2, or 3)
ALTER TABLE reports ADD COLUMN IF NOT EXISTS active_phase INT;
...
CREATE TABLE ai_logs (
    ...
    action VARCHAR(100) NOT NULL, -- e.g., 'PHASE_1_EXTRACTION', 'PHASE_2_JUDGMENT', 'PHASE_3_DRAFT'
    model VARCHAR(100) NOT NULL,
    
    prompt_snapshot TEXT, -- The full input sent to AI
    ai_response_snapshot TEXT, -- The raw output from AI
...
-- Phase 1: Link Evidence to the AI Log that extracted it
ALTER TABLE evidence 
ADD COLUMN IF NOT EXISTS ai_log_id UUID REFERENCES ai_logs(id) ON DELETE SET NULL;

-- Phase 2: Link Analysis parts to their specific AI Tasks
-- (We separate them because they are generated by different prompts/calls)
ALTER TABLE competency_analysis 
ADD COLUMN IF NOT EXISTS narrative_log_id UUID REFERENCES ai_logs(id) ON DELETE SET NULL,
ADD COLUMN IF NOT EXISTS recommendations_log_id UUID REFERENCES ai_logs(id) ON DELETE SET NULL;

ALTER TABLE analysis_key_behaviors
ADD COLUMN IF NOT EXISTS judgment_log_id UUID REFERENCES ai_logs(id) ON DELETE SET NULL;

-- Phase 3: Link Summary to the Final Critique Log
ALTER TABLE executive_summary
ADD COLUMN IF NOT EXISTS ai_log_id UUID REFERENCES ai_logs(id) ON DELETE SET NULL;